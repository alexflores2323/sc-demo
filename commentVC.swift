...
//  insta
//
//  Created by Logan Caracci on 3/5/17.
...
class commentVC: UIViewController, UITextViewDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    func loadComments() {
        ...
        countQuery.countObjectsInBackground (block: { (count, error) -> Void in
            ...
            if self.page < count {
                self.refrsher.addTarget(self, action: #selector(commentVC.loadMore), for: UIControlEvents.valueChanged)
                self.tableView.addSubview(self.refrsher)
            }
    ...
    func loadMore() {
        ...
        countQuery.countObjectsInBackground (block: { (count, error) -> Void in
            ...
            // self refresher
            self.refrsher.endRefreshing()
            
            // remove refresher if loaded all comments
            if self.page >= count {
                self.refrsher.removeFromSuperview()
            }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Cell") as! commentCellVC
    ...
    // when they click username button
    @IBAction func usernameBtn_click(_ sender: Any) {
        ...
        let cell = tableView.cellForRow(at: i) as! commentCellVC
    ...
    // swipe cell for actions delete, complain, address...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        
       

        // call cell for calling further cell data
        let cell = tableView.cellForRow(at: indexPath) as! commentCellVC
